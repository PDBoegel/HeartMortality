...
# ABSTRACT
  ...
  This report looked to bring into light and examine how United States County-based Socioeconomic
...
# INTRODUCTION
  ...
  In the United States, an average of 630,000 people per year die from Heart Disease- accounting for
...
sources of error, and offer alterations for future studies.
  ...
  The problem of Heart Disease is extremely prevalent, it being the leading cause of both male and
...
# DATA
  ...
  The data in this analysis comes from reliable sources within the U.S. government. Reports from the
...
consolidate all the variables of interest into one csv file.
  ...
  While the initial sample size was for n = 3200 counties, after merging all of the necessary
...
insignificant to the overall analysis performed because of the large amount of available data.
  ...
  It is noteworthy that the year of one of the data sheets is different. This distinction is trivial
...
States.
  ...
  The main outcome of interest is an individual’s chance of dying from Heart Disease based on certain
...
understanding of how general socioeconomic factors affect heart disease mortality chances.
  ...
  Table 1 provides summary statistics for each variable.  After observing the maximum and minimum
...
deviations from the mean.
  ...
  To try to compensate for this reality, two solutions were attempted in the analysis. The first
...
percentages can be relatively high even if the county population is extremely small.
  ...
  Histograms 1 and 2 display the percentage of African Americans in each county compared to the
...
disregarding the significance of raw numerical totals compared to percentages.
  ...
  Several groups of explanatory variables presented serious collinearity concerns.  The data on the
...
chance of heart disease mortality.
  ...
  To find unusual points in the data, scripts were written to find high leverage points and outliers.
A leverage plot was created to visually represent leverage values for all observations. (Refer to an
image of the leverage plot in the Appendix) Once we found the average leverage value, 96 observations
...
0.5372 to 0.5185 - a 3.5% decrease.
  ...
  Similar steps were followed to detect outliers in the data. First, a standardized residual plot was
...
it is noteworthy that no points in the model were both outliers and high leverage points (Appendix).
...
and outliers resulted in an R^2 value of 0.5951 - a 10.8% increase from the original regression.
  ...
  Through this sensitivity analysis, this report concluded that the model is relatively insensitive
...
# EMPIRICAL ANALYSIS
  ...
  The data was compiled into three preliminary models: a model with categorical data (the Category
...
F-Test on only the statistically significant, the desired reduced model was created.
  ...
  From the reduced model, the PCT Model, this report will comment on the data’s anomalies and
...
# CONCLUDING REMARKS
  ...
  Throughout the direction of this report, the aim has been to provide sound significant evidence (or
