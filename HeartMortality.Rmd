...
heartmort <- subset(heartmort, AVGINCOMEPERPERSON > 0) # 3 counties had 0 avgincome, removal should not affect study and makes histogram/log transform variables better later
summaryOfVars <- summary(heartmort)
summaryOfVars
```
```{r, echo=FALSE}
heartmort <- heartmort[,c(1:6,14:16,19,7:13,18,20,17)] #Organization to make category variables easier later on. Does not need to go into document
heartmort[,c(10:19)] <- heartmort[,c(10:19)]/100 # Making percents into numbers [0,1]
```
```{r, results='hide'}
par(mfrow = c(2,3))
...
Some data in AVGINCOMEPERPERSON is zero which is strange might want to consider removing these points    
```{r, results='hide'}
<<<<<<< HEAD
...
>>>>>>> 644dbb61a5d71eadd146eb15e3bf2a8667a66df7
par(mfrow = c(2,3))
