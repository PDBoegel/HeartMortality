...
```{r}
totlm = lm(LNHDMORTPERPERSON ~ 1, data = heartmort)
summary(totlm)
```

```{r}
catlm = lm(LNHDMORTPERPERSON ~ 1, data = heartmort)
summary(catlm)
```
...
Create possible ANOVA model and assess if any (residuals)^2 account for a significant portion of ESS
